...
class WXRImporter extends \WP_Importer {
	...
	// path for image download from local
	protected $path = '';
	// replace link if not image
	protected $url = '';
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			...
			$remote_url = $this->checkandreturnnewimageurl( $remote_url );
			...
			if ( preg_match_all( '@src="([^"]+)"@', $htmlString, $match ) ) {
				$srcs       = array_pop( $match );
				$domainname = apply_filters( 'pt-ocdi/domain_name', 'smartdata' );
				foreach ( $srcs as $src ) {

					if ( $this->strpos_arr( $src, $domainname ) !== false ) {
						// $this->write_log("\n" . ' remote URL:-' . $src);
						$htmlString               = $this->insert_remote_image( $src, $htmlString );
						$data['post_content']     = $htmlString;
						$postdata['post_content'] = $htmlString;
					}
				}
			}

			$this->url = apply_filters( 'pt-ocdi/replace_url', $this->url );
			$site_url  = rtrim( site_url() );
			foreach ( $this->url as $url ) {
				$postdata = str_replace( $url, $site_url, $postdata );
			}
	...
	/**
	 * Attempt to change remote url
	 *
	 * @param array $remote_url
	 */
	public function checkandreturnnewimageurl( $remote_url ) {
		if ( strpos( $remote_url, 'wp-content' ) !== false ) {
			$urlpath    = substr_replace( $remote_url, '', 0, strpos( $remote_url, 'wp-content' ) + 11 );
			$this->path = apply_filters( 'pt-ocdi/destination_path', $this->path );
			// $this->write_log("\n" . 'checkandreturnnewimageurl:-' . $remote_url);
			if ( $this->path != '' ) {

				if ( file_exists( ABSPATH . $this->path . $urlpath ) ) {
					$site_url   = rtrim( site_url(), '/' );
					$remote_url = $site_url . $this->path . $urlpath;
	...
	public function my_custom_upload_dir( $upload ) {

		$urlpath = substr_replace( $this->image_url, '', 0, strpos( $this->image_url, 'uploads' ) + 7 );
		$subdir  = substr( $urlpath, 0, strrpos( $urlpath, '/' ) );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		// echo $remote_url;
		// echo "<br>";
		$this->image_url = $remote_url;
		if ( strpos( $remote_url, 'revslider' ) !== false ) {
			// $this->write_log("\n" . 'revslider=' . $remote_url);
			add_filter( 'upload_dir', array( $this, 'my_custom_upload_dir' ) );
		}

		$upload = $this->fetch_remote_file( $remote_url, $post );
		if ( strpos( $remote_url, 'revslider' ) !== false ) {
			// $this->write_log("\n" . 'revslider=' . $remote_url);
			remove_filter( 'upload_dir', array( $this, 'my_custom_upload_dir' ) );
		}
		...
		update_post_meta( $post_id, '_smartdata_source_image_hash', $this->get_hash_image( $remote_url ) );
		return $post_id;
	}
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			if ( $meta_item['key'] == '_elementor_data' ) {
				preg_match_all( '/"url"\:\"([^\"]+)\"/', $meta_item['value'], $meta_item_values );
				$meta_item_html = $meta_item['value'];
				if ( ! empty( $meta_item_values[1] ) ) {
					foreach ( $meta_item_values[1] as $meta_item_value ) {
						if ( $this->strpos_arr( $meta_item_value, $domainname ) !== false ) {
							$meta_item_html_new = $this->replace_remote_uri( $meta_item_value, $meta_item_html );
							$meta_item_html_new = $this->replace_remote_image_uri( $meta_item_value, $meta_item_html );
							if ( $meta_item_html_new ) {
								$meta_item_html = $meta_item_html_new;
							}
						}
					}
				}
				$meta_item['value'] = $meta_item_html;
			}

			if ( $meta_item['key'] == '_menu_item_url' ) {
				// print_r( $meta_item['value']);
				if ( $this->strpos_arr( $meta_item['value'], $domainname ) !== false ) {
					$meta_item_html     = $this->replace_remote_uri( $meta_item['value'], $meta_item['value'] );
					$meta_item['value'] = $meta_item_html;
					// print_r("new:-". $meta_item['value']);
				}
				// exit();
	...
	private function get_saved_image( $attachment, $metakey ) {
		global $wpdb;
			...
			$wpdb->prepare(
				'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
					WHERE `meta_key` = \'' . $metakey . '\'
						AND `meta_value` = %s
				;',
				$this->get_hash_image( $attachment )
			)
		);
	...
	protected function replace_remote_image_uri( $remote_url, $html ) {
		$remote_url_new = stripslashes( $remote_url );
		$this->url      = apply_filters( 'pt-ocdi/replace_url', $this->url );
		$site_url       = rtrim( site_url(), '/' );
		foreach ( $this->url as $url ) {
			if ( strpos( $remote_url_new, $url ) !== false ) {
				$urlpath_url = substr_replace( $remote_url_new, '', 0, strpos( $remote_url_new, $url ) + strlen( $url ) + 1 );
				$html        = str_replace( $remote_url, addslashes( $site_url . '/' . $urlpath_url ), $html );
			}
		}
		return $html;
	}

	protected function replace_remote_uri( $remote_url, $html ) {

		$remote_url_new = stripslashes( $remote_url );
		// $this->write_log('replace_remote_uri preocess:-' . remote_url_new);
		$this->url = apply_filters( 'pt-ocdi/replace_url', $this->url );
		if ( ! preg_match( '/\.(png|jpeg|jpg|gif|bmp)$/i', $remote_url_new ) && $this->url != '' ) {
			$site_url = rtrim( site_url(), '/' );
			foreach ( $this->url as $url ) {
				if ( strpos( $remote_url_new, $url ) !== false ) {
					$urlpath_url = substr_replace( $remote_url_new, '', 0, strpos( $remote_url_new, $url ) + strlen( $url ) + 1 );
					// $this->write_log('replace_remote_uri preocess:-' . $site_url . '/' . $urlpath_url);
					$html = str_replace( $remote_url, addslashes( $site_url . '/' . $urlpath_url ), $html );
				}
			}
	...
	protected function insert_remote_image( $remote_url, $html, $type = '' ) {
		$html_with_replace_link = $this->replace_remote_uri( $remote_url, $html );
		if ( $html_with_replace_link ) {
			return $html_with_replace_link;
		}
		$remote_url_s = stripslashes( $remote_url );
		// $this->write_log("\n" . 'Main URL:-' . $remote_url_s);
		if ( strpos( $remote_url_s, 'wp-content' ) !== false ) {
			$remote_url_s = $this->checkandreturnnewimageurl( $remote_url_s );
			// $this->write_log('Main URL after preocess:-' . $remote_url_s);
			$saved_image = $this->get_saved_image( $remote_url_s, '_smartdata_source_image_hash' );
			// $this->write_log("\n" . 'Save Image:-' . $saved_image);
			if ( $saved_image ) {
				$html = str_replace( $remote_url, addslashes( $saved_image ), $html );
				// $this->write_log("\n" . 'Replace This Image:-' . $saved_image);
				return $html;
			}
			$file = $this->get_image_temp_file( $remote_url_s );
			if ( $file ) {
				$overrides = array(
					'test_form' => false,
					'test_size' => true,
				...
				$results       = $this->move_image_temp_file_to_uploaddir( $file );
				$attachment_id = $this->move_image_uploaddir_to_attachment( $results );
				if ( $attachment_id ) {
					$this->upload_image_hash_to_postmeta( $type, $attachment_id, $remote_url );
					if ( ! empty( $results['error'] ) ) {
					} else {
						$local_url = $results['url']; // URL to the file in the uploads dir
						$html      = str_replace( $remote_url, addslashes( $local_url ), $html );
	...
	private function upload_image_hash_to_postmeta( $type, $attachment_id, $remote_url ) {

		if ( $type == 'elementor' ) {
			update_post_meta( $attachment_id, '_elementor_source_image_hash', $this->get_hash_image( $remote_url ) );
			update_post_meta( $attachment_id, '_smartdata_source_image_hash', $this->get_hash_image( $remote_url ) );
		} else {
			update_post_meta( $attachment_id, '_smartdata_source_image_hash', $this->get_hash_image( $remote_url ) );
		}
	}

	private function move_image_temp_file_to_uploaddir( $file ) {
	...
	private function move_image_uploaddir_to_attachment( $results ) {
		...
		$attachment_id   = wp_insert_attachment( $attachment_data, $results['file'] );
		if ( ! $attachment_id ) {
			return;
		}
	...
	private function get_image_temp_file( $remote_url ) {
		$timeout_seconds = 30;
		$temp_file       = download_url( $remote_url, $timeout_seconds );
		$ext             = pathinfo( $remote_url, PATHINFO_EXTENSION );
		if ( ! is_wp_error( $temp_file ) ) {
			$file = array(
				'name'     => basename( $remote_url ), // ex: wp-header-logo.png
				'type'     => 'image/' . $ext,
				'tmp_name' => $temp_file,
				'error'    => 0,
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning(
						sprintf(
							__( 'Failed to add metakey: %1$s, metavalue: %2$s to term_id: %3$d', 'wordpress-importer' ),
							$key,
							$value,
							$term_id
						)
					...
					$this->logger->debug(
						sprintf(
							__( 'Meta for term_id %1$d : %2$s => %3$s ; successfully added!', 'wordpress-importer' ),
							$term_id,
							$key,
							$value
						)
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				$this->logger->warning(
					sprintf(
						__( 'The term(%1$d)"s parent_slug (%2$s) is not found in the remapping-array.', 'wordpress-importer' ),
						$term_id,
						$parent_slug
					)
				);
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (term #%2$d) with mapped data', 'wordpress-importer' ),
						$termattributes['name'],
						$term_id
					)
				);
			...
			$this->logger->debug(
				sprintf(
					__( 'Term %1$d was successfully updated with parent %2$d', 'wordpress-importer' ),
					$term_id,
					$mapped_parent
				)
			);
	...
	function remap_featured_images() {
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					$this->logger->info( sprintf( esc_html__( 'Remapping featured image ID %1$d to new ID %2$d for post ID %3$d', 'wordpress-importer' ), $value, $new_id, $post_id ) );
