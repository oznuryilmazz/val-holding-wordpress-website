		...
		<?php if (!empty($form -> form_fields)) : ?>
			<form class="<?php echo (empty($form_styling['formlayout']) || $form_styling['formlayout'] == "normal") ? ((!empty($form_styling['twocolumns'])) ? 'form-twocolumns' : 'form-onecolumn') : (($form_styling['formlayout'] == "inline") ? 'form-inline' : 'form-horizontal'); ?> newsletters-subscribe-form <?php echo (!empty($form -> ajax)) ? 'newsletters-subscribe-form-ajax' : 'newsletters-subscribe-form-regular'; ?>" action="<?php echo ((empty($action)) ? $Html -> retainquery($this -> pre . 'method=optin') : $action); ?>#newsletters-<?php echo esc_html($form -> id); ?>-form" method="post" id="newsletters-<?php echo esc_html($form -> id); ?>-form" enctype="multipart/form-data">
					...
					<?php if ($captcha_type = $this -> use_captcha()) : ?>		
						<?php if ($captcha_type == "rsc") : ?>
							<div class="form-group<?php echo (!empty($errors['captcha_code'])) ? ' has-error' : ''; ?> newsletters-fieldholder newsletters-captcha newsletters-captcha-wrapper">
					            ...
					            <?php if (!empty($errors['captcha_code']) && !empty($form_styling['fielderrors'])) : ?>
									<div id="newsletters-<?php echo esc_html($form -> id); ?>-captcha-error" class="newsletters-field-error alert alert-danger my-1">
						...
						<?php elseif ($captcha_type == "recaptcha") : ?>
							...
							<div class="newsletters-captcha-wrapper form-group newsletters-fieldholder <?php echo ($recaptcha_type == "invisible") ? 'newsletters-fieldholder-hidden hidden' : ''; ?>">
								<div id="newsletters-<?php echo esc_html($form -> id); ?>-recaptcha-challenge" class="newsletters-recaptcha-challenge my-1"></div>
								
								<?php if (!empty($errors['captcha_code']) && !empty($form_styling['fielderrors'])) : ?>
									<div id="newsletters-<?php echo esc_html($form -> id); ?>-captcha-error" class="newsletters-field-error alert alert-danger">
										<i class="fa fa-exclamation-triangle"></i> <?php echo wp_kses_post( wp_unslash($errors['captcha_code'])) ?>
									</div>
								<?php endif; ?>
							</div>
				...
				#newsletters-<?php echo esc_html($form -> id); ?>-form .form-control {
					...
					<?php echo (isset($form_styling['fieldborderradius']) && (!empty($form_styling['fieldborderradius']) || $form_styling['fieldborderradius'] == 0)) ? 'border-radius: ' . $form_styling['fieldborderradius'] . 'px;' : ''; ?>
				}
				...
				#newsletters-<?php echo esc_html($form -> id); ?>-form .btn {
					<?php echo (isset($form_styling['buttonbordersize']) && (!empty($form_styling['buttonbordersize']) || $form_styling['buttonbordersize'] == 0)) ? 'border-width: ' . $form_styling['buttonbordersize'] . 'px;' : ''; ?>
					<?php echo (isset($form_styling['buttonborderradius']) && (!empty($form_styling['buttonborderradius']) || $form_styling['buttonborderradius'] == 0)) ? 'border-radius: ' . $form_styling['buttonborderradius'] . 'px;' : ''; ?>
				}
				...
				#newsletters-<?php echo esc_html($form -> id); ?>-form .select2-container--default .select2-selection--single {
					<?php echo (!empty($form_styling['fieldcolor'])) ? 'background-color: ' . $form_styling['fieldcolor'] . ';' : ''; ?>
					<?php echo (isset($form_styling['fieldborderradius']) && (!empty($form_styling['fieldborderradius']) || $form_styling['fieldborderradius'] == 0)) ? 'border-radius: ' . $form_styling['fieldborderradius'] . 'px;' : ''; ?>
				}
				
				#newsletters-<?php echo esc_html($form -> id); ?>-form .select2-container .select2-selection--single .select2-selection__rendered {
					<?php echo (!empty($form_styling['fieldtextcolor'])) ? 'color: ' . $form_styling['fieldtextcolor'] . ';' : ''; ?>
				}
