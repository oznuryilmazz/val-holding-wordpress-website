...
<?php if (!empty($checkoutdata)) : ?>
	...
	<?php endif; ?>	
		<?php foreach ($checkoutdata as $ckey => $cval) : ?>
			<input type="hidden" name="<?php echo esc_html($ckey); ?>" value="<?php echo esc_html($cval); ?>" />
		<?php endforeach; ?>
		<?php $buttontext = (empty($extend)) ? __('Pay with PayPal', 'wp-mailinglist') : __('Extend with PayPal', 'wp-mailinglist'); ?>
		<button value="1" type="submit" class="<?php echo esc_html($this -> pre); ?>button ui-button-success paybutton" name="checkout">
